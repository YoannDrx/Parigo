...
La traduction générique des recherches françaises sans résultat utilise DeepL côté serveur. Ajouter `DEEPL_AUTH_KEY` pour l’activer ; une clé API Free (`:fx`) sélectionne automatiquement `https://api-free.deepl.com`, sinon l’endpoint Pro est utilisé. `DEEPL_API_URL` permet uniquement de remplacer explicitement cet endpoint. DeepL retourne une suggestion que l’utilisateur doit accepter : la requête littérale n’est jamais remplacée automatiquement. Après acceptation, la traduction remplace le texte du champ, devient la nouvelle requête littérale et rouvre l’autocomplétion avec le focus conservé. Sur l’accueil, une autocomplétion vide déclenche un contrôle léger `probe=1` ; ce contrôle vérifie le zéro résultat global sans enregistrer une recherche dans l’historique membre. Sans clé, la recherche Harvest continue de fonctionner sans suggestion traduite.

Le mode « Brief IA » est réservé à AIMS. Il peut être sélectionné depuis l’icône placée dans la barre afin de prévisualiser l’interface, mais son envoi reste désactivé. `/api/search/capabilities` expose cette indisponibilité ; aucune recherche par intention locale n’est exécutée et aucun secret AIMS n’est attendu dans cette livraison. La barre de l’accueil parcourt universellement pistes, albums, playlists et métadonnées ; le choix Pistes/Albums est rattaché aux contrôles de la page de résultats. L’autocomplétion s’ouvre sous la barre sur toute sa largeur et réutilise les pochettes Harvest via leurs gabarits d’assets, sans requête d’hydratation par suggestion. Les paroles restent exclues du profil éditorial général : une recherche `TrackLyrics` isolée, limitée et dédupliquée n’est lancée que pour une expression précise ou lorsque les suggestions éditoriales sont rares.
