...
import type { Album, Label, ViewMode } from "@/types";
...
interface AlbumsPageClientProps {
  initialAlbums: { albums: Album[]; pagination: { total: number; limit: number; offset: number; hasMore: boolean } };
  initialLabels: { labels: Label[]; pagination: { total: number; limit: number; offset: number; hasMore: boolean } };
  initialGenres: { genres: Array<{ id: string; name: string; slug: string }> };
}

export function AlbumsPageClient({ initialAlbums, initialLabels, initialGenres }: AlbumsPageClientProps) {
  ...
  }, true, selectedLabel === null && selectedGenre === null ? initialAlbums : undefined);
  const { data: labelsData } = useLabels({ limit: 20 }, initialLabels);
  const { data: genresData } = useGenres(initialGenres);
              ...
              >
                {sortedAlbums.map((album, index) => (
                  <div
                    key={album.id}
                  >
                    {viewMode === "grid" ? <AlbumCard album={album} priority={index === 0} /> : (
                      <Link href={localizedPath(`/albums/${album.slug || album.id}`)} className="group grid min-h-28 grid-cols-[5rem_minmax(0,1fr)_auto] items-center gap-4 border-b border-[var(--line)] py-4 last:border-b-0 transition-colors hover:bg-[var(--surface-soft)] sm:min-h-32 sm:grid-cols-[6rem_minmax(0,1fr)_auto] sm:gap-6 sm:px-4">
