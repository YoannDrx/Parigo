  ...
  return (
    <div className="page-shell flex min-h-screen flex-col">
      ...
      <main className="flex-1 pb-[var(--space-page-end)] pt-[70px]">
        ...
        <section className="editorial-detail-hero relative mx-auto grid max-w-[1700px] gap-[var(--space-block-gap)] overflow-hidden px-[var(--space-page-gutter)] pb-0 pt-[var(--space-section-y)] md:grid-cols-12">
          <div className="parigo-frame relative aspect-[16/10] min-w-0 overflow-hidden border border-[var(--line)] bg-[linear-gradient(145deg,color-mix(in_srgb,var(--signal)_8%,var(--surface)),var(--surface)_62%)] p-5 shadow-[7px_8px_0_color-mix(in_srgb,var(--signal)_11%,transparent)] sm:p-8 md:col-span-5 md:aspect-auto md:min-h-[34rem] lg:p-10"><div className="relative min-h-full w-full"><LabelLogo src={label.logo} name={label.name} fill sizes="(max-width: 768px) 92vw, 42vw" fallbackSize={144} priority className="object-contain" /></div></div>
          <div className="min-w-0 self-center md:col-span-6 md:col-start-7"><SignedTitle className="[overflow-wrap:anywhere] font-[var(--font-editorial)] text-[clamp(3rem,9vw,10rem)] font-normal leading-[.76] tracking-[-.065em]">{label.name}</SignedTitle><div className="mt-[var(--space-block-gap)] flex flex-wrap gap-8 border-y border-[var(--line)] py-5 font-mono text-[.65rem] uppercase tracking-[.1em] text-[var(--text-muted)]"><span>{label.albumCount} {label.albumCount === 1 ? t("catalog.album") : t("catalog.albums")}</span><span>{label.trackCount} {label.trackCount === 1 ? t("catalog.track") : t("catalog.tracks")}</span></div>{label.description && <p className="mt-[var(--space-block-gap)] max-w-2xl text-lg leading-relaxed text-[var(--text-muted)]">{label.description}</p>}</div>
        </section>
