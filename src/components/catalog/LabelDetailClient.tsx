...
import { resolveLocalizedValue } from "@/lib/catalog-localization";
...
export function LabelDetailClient({ label }: { label: LabelDetail }) {
  ...
  const description = resolveLocalizedValue(label.descriptions, label.description, locale);
    ...
    <div className="page-shell flex min-h-screen flex-col">
      ...
      <main className="flex-1 pb-[var(--space-page-end)] pt-[70px]">
        ...
        <section className="editorial-detail-hero relative mx-auto grid max-w-[1700px] gap-[var(--space-block-gap)] overflow-hidden px-[var(--space-page-gutter)] pb-0 pt-[var(--space-section-y)] md:grid-cols-12">
          ...
          <div className="min-w-0 self-center md:col-span-8 md:col-start-5"><SignedTitle className="[overflow-wrap:anywhere] font-[var(--font-editorial)] text-[clamp(3rem,9vw,10rem)] font-normal leading-[.76] tracking-[-.065em]">{label.name}</SignedTitle><div className="mt-[var(--space-block-gap)] flex flex-wrap gap-8 border-y border-[var(--line)] py-5 font-mono text-[.65rem] uppercase tracking-[.1em] text-[var(--text-muted)]"><span>{label.albumCount} {label.albumCount === 1 ? t("catalog.album") : t("catalog.albums")}</span><span>{label.trackCount} {label.trackCount === 1 ? t("catalog.track") : t("catalog.tracks")}</span></div>{description && <p className="mt-[var(--space-block-gap)] max-w-2xl text-lg leading-relaxed text-[var(--text-muted)]">{description}</p>}</div>
        </section>
