...
export function SynchronisationCard({
  ...
  return (
    <article className={cn("home-sync-card editorial-video-card group block min-w-0 max-md:before:!hidden max-md:after:!hidden", className)}>
      ...
      <footer className="editorial-card__mobile-footer relative z-[1] flex min-h-24 items-end justify-between gap-4 px-1 pb-1 pt-5 text-[var(--foreground)] md:hidden">
        <div className="min-w-0">
          <p className="truncate font-mono text-[.54rem] uppercase tracking-[.12em] text-[var(--signal-strong)]">{mobileEyebrow}</p>
          <Heading className="mt-2 line-clamp-2 text-lg font-semibold leading-[1.05] tracking-[-.025em]">
            {title}
          </Heading>
        </div>
        {detail ? <p className="shrink-0 font-mono text-[.55rem] text-[var(--text-muted)]">{detail}</p> : null}
      </footer>
