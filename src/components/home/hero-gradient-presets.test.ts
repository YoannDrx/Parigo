...
describe("home hero Gradflow presets", () => {
  it("keeps four explicit Silk presets", () => {
    expect(Object.values(HERO_GRADIENT_PRESETS).flatMap((theme) => Object.values(theme))).toHaveLength(4);
    expect(Object.values(HERO_GRADIENT_PRESETS).flatMap((theme) => Object.values(theme)).every((preset) => preset.type === "silk")).toBe(true);
  });

  it("uses the requested moderate-speed, maximum-scale Silk setup in every preset", () => {
    const presets = Object.values(HERO_GRADIENT_PRESETS).flatMap((theme) => Object.values(theme));
    expect(presets.every((preset) => preset.speed === 0.7)).toBe(true);
    expect(presets.every((preset) => preset.scale === 3)).toBe(true);
    expect(presets.every((preset) => preset.noise === 0)).toBe(true);
  });

  it("keeps saturated light-mode silk colors behind dark hero copy", () => {
