...
function EditorialCard({ playlist, index, progress, total, locale }: { playlist: Playlist; index: number; progress: MotionValue<number>; total: number; locale: "fr" | "en" }) {
  ...
  return (
    <motion.div style={{ y, rotate, scale }} className="w-full shrink-0 md:w-[min(30vw,430px)]">
      <Link href={`/playlists/${playlist.id}`} className="group block overflow-hidden rounded-[1.4rem] bg-[#f1efe7] p-3 shadow-[0_32px_90px_rgba(0,0,0,.28)] transition-shadow hover:shadow-[0_40px_110px_rgba(0,0,0,.42)] md:p-4">
        <div className="relative aspect-square overflow-hidden rounded-[1rem] bg-[#e3e3dc]">
          ...
          <span className="absolute left-4 top-4 rounded-full bg-[#111510]/82 px-3 py-1.5 font-mono text-[.58rem] tracking-[.13em] text-white backdrop-blur-md">PARIGO / {String(index + 1).padStart(2, "0")}</span>
        </div>
...
export function EditorialScrollStory({ playlists, locale }: { playlists: Playlist[]; locale: "fr" | "en" }) {
  ...
  return (
    <section ref={sectionRef} className="relative min-h-[100svh] overflow-clip bg-[#101511] px-4 py-20 text-[#f3f0e8] md:min-h-[245svh] md:px-8 md:py-0">
      ...
      <div className="mx-auto grid max-w-[1580px] gap-12 md:sticky md:top-[70px] md:h-[calc(100svh-70px)] md:grid-cols-[minmax(260px,34%)_1fr] md:items-center md:overflow-hidden">
        <div className="relative z-10 md:pr-10">
          ...
          <p className="mt-6 max-w-sm leading-relaxed text-white/58">{locale === "fr" ? "Chaque pochette ouvre un angle narratif. Faites défiler la collection, puis entrez dans l’univers qui répond à votre brief." : "Each cover opens a narrative angle. Scroll through the collection, then enter the world that answers your brief."}</p>
          ...
          <p className="mt-4 hidden font-mono text-[.58rem] uppercase tracking-[.12em] text-white/35 md:block">{locale === "fr" ? "Le scroll compose la séquence" : "Scroll composes the sequence"}</p>
        </div>
        <div className="min-w-0 overflow-hidden">
          <motion.div style={reduceMotion ? undefined : { x }} className="flex flex-col gap-4 md:w-max md:flex-row">
