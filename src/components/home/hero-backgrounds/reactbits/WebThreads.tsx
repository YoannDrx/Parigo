...
/*
 * Adapted from React Bits (WebThreads, TS-TW variant).
 * Copyright (c) 2026 David Haz.
 * Licensed under the React Bits MIT + Commons Clause License Condition v1.0.
 * Source: https://reactbits.dev/backgrounds/web-threads
 */
...
void main() {
  ...
  if (uLightMode) {
    ...
    float coverage = smoothstep(0.08, 0.48, rawEnergy);
    coverage *= coverage;
...
const WebThreads: React.FC<WebThreadsProps> = ({
  ...
  useEffect(() => {
    ...
    const interactionSurface = container.closest<HTMLElement>('.home-hero') ?? canvas;
    interactionSurface.addEventListener('mousemove', onMouseMove);
    interactionSurface.addEventListener('mouseenter', onMouseEnter);
    ...
    return () => {
      ...
      interactionSurface.removeEventListener('mouseenter', onMouseEnter);
