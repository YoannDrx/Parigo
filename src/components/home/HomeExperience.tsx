...
import { AlertCircle, ArrowDown, ArrowRight, ArrowUpRight, Facebook, Instagram, Linkedin, Play, RotateCcw, Youtube } from "lucide-react";
      ...
      <main>
        <section className="relative mt-[74px] flex min-h-[calc(100svh-74px)] items-center overflow-hidden bg-[var(--surface)] px-4 py-10 md:px-8 md:py-12">
          ...
          <div className="pointer-events-none relative mx-auto w-full max-w-[1180px] text-center">
            ...
            <p className="mx-auto mt-6 max-w-3xl font-[var(--font-rounded)] text-base leading-relaxed text-[var(--text-muted)] md:text-lg">
              A curated catalogue built for editors, music supervisors and producers.<br className="hidden sm:block" />
              Search, listen, compare and license — without the noise.
            </p>
            <div className="pointer-events-auto mx-auto mt-9 max-w-4xl text-left"><AISearch /></div>
            <a href="#about" className="pointer-events-auto mx-auto mt-11 inline-flex min-h-9 items-center gap-2 font-mono text-[.56rem] uppercase tracking-[.16em] text-[var(--text-muted)]/65 transition hover:text-[var(--foreground)]">{t("home.scroll")} <ArrowDown size={12} /></a>
          </div>
        </section>
          ...
          <SectionReveal className="mx-auto max-w-[1580px]">
            <div className="relative min-h-[610px] overflow-hidden rounded-xl md:min-h-[760px]">
              ...
              <div className="absolute inset-0 flex max-w-3xl flex-col justify-end p-6 text-white md:p-14 lg:p-20">
                ...
                <Link href="/albums" className="mt-8 inline-flex min-h-11 w-fit items-center gap-2 rounded-md border border-white/65 px-5 text-sm font-semibold text-white transition hover:bg-white hover:text-[#111510]">{locale === "fr" ? "Découvrir le catalogue" : "Explore the catalogue"}<ArrowRight size={15} /></Link>
              </div>
            </div>
          </SectionReveal>
        </section>
        ...
        <section className="px-4 pb-20 md:px-8 md:pb-28">
          <SectionReveal className="group relative mx-auto grid max-w-[1580px] overflow-hidden rounded-[1.2rem] bg-[var(--signal-strong)] p-6 text-white md:grid-cols-12 md:items-center md:p-10 lg:p-14">
            ...
            <div className="relative md:col-span-9 md:flex md:items-center md:gap-4"><div className="relative h-28 w-full max-w-[15rem] shrink-0" aria-label={locale === "fr" ? "Plateformes Parigo : Instagram, YouTube, LinkedIn, Facebook, Bandcamp, TikTok et Spotify" : "Parigo platforms: Instagram, YouTube, LinkedIn, Facebook, Bandcamp, TikTok and Spotify"}>{LINKTREE_PLATFORMS.map((platform) => <span key={platform.name} aria-label={platform.name} className={`absolute flex h-12 w-12 items-center justify-center rounded-[.9rem] border border-white/70 bg-[#ffffff] text-[#247b43] shadow-[0_12px_32px_rgba(19,70,37,.2)] transition-transform duration-500 ${platform.position}`}><PlatformIcon name={platform.name} /><span className="sr-only">{platform.name}</span></span>)}</div><div className="relative mt-4 md:mt-0"><p className="eyebrow text-white/62">Parigo ailleurs</p><h2 className="mt-3 text-[clamp(2rem,4vw,4.5rem)] leading-[.94]">{locale === "fr" ? "Suivez le fil Parigo." : "Follow the Parigo signal."}</h2><p className="mt-4 max-w-xl text-sm leading-relaxed text-white/68">{locale === "fr" ? "Sorties, playlists, images et actualités du studio — tous nos liens réunis au même endroit." : "Releases, playlists, images and studio news — all our links in one place."}</p></div></div>
