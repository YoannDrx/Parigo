      ...
      <main>
        ...
        <section data-testid="social-follow-section" className="px-4 py-20 md:px-8 md:py-28">
          <SectionReveal className="group relative mx-auto grid max-w-[1580px] overflow-hidden rounded-[1.2rem] bg-[var(--signal-strong)] p-6 text-white md:grid-cols-12 md:items-center md:p-10 lg:p-14">
            ...
            <div className="relative md:col-span-9 md:flex md:items-center md:gap-4"><div className="relative h-28 w-full max-w-[15rem] shrink-0" role="list" aria-label={locale === "fr" ? "Plateformes Parigo : Instagram, YouTube, LinkedIn, Facebook, Bandcamp, TikTok et Spotify" : "Parigo platforms: Instagram, YouTube, LinkedIn, Facebook, Bandcamp, TikTok and Spotify"}>{LINKTREE_PLATFORMS.map((platform) => <span key={platform.name} role="listitem" className={`absolute flex h-12 w-12 items-center justify-center rounded-[.9rem] border border-white/70 bg-[#ffffff] text-[#247b43] shadow-[0_12px_32px_rgba(19,70,37,.2)] transition-transform duration-500 ${platform.position}`}><PlatformIcon name={platform.name} /><span className="sr-only">{platform.name}</span></span>)}</div><div className="relative mt-4 text-[#123f24] md:mt-0"><p className="eyebrow">{locale === "fr" ? "Parigo ailleurs" : "Parigo elsewhere"}</p><h2 className="mt-3 text-[clamp(2rem,4vw,4.5rem)] leading-[.94]">{locale === "fr" ? "Suivez le fil Parigo." : "Follow the Parigo signal."}</h2><p className="mt-4 max-w-xl text-sm leading-relaxed">{locale === "fr" ? "Sorties, playlists, images et actualités du studio — tous nos liens réunis au même endroit." : "Releases, playlists, images and studio news — all our links in one place."}</p></div></div>
