  ...
  return (
    ...
    >
      ...
      <div className="parigo-track-row__actions flex flex-shrink-0 items-center gap-0.5">
        ...
        <Tooltip label={locale === "fr" ? "Informations sur la piste" : "Track information"}><button type="button" onClick={() => toggleDetails("information")} aria-expanded={detailsOpen} className={cn("flex h-10 w-10 items-center justify-center transition hover:bg-[var(--surface-soft)]", detailsOpen && "text-[var(--signal-strong)]")} aria-label={`${locale === "fr" ? "Informations sur la piste" : "Track information"} : ${track.title}`}><Info size={17} /></button></Tooltip><SimilarTracksButton trackId={track.id} trackTitle={track.title} />{session?.user && <Tooltip label={locale === "fr" ? "Note privée" : "Private note"}><button type="button" onClick={() => toggleDetails("notes")} aria-expanded={detailsOpen && detailsTab === "notes"} className={cn("flex h-10 w-10 items-center justify-center transition hover:bg-[var(--surface-soft)]", detailsOpen && detailsTab === "notes" && "text-[var(--signal-strong)]")} aria-label={`${locale === "fr" ? "Ouvrir les notes privées" : "Open private notes"} : ${track.title}`}><NotebookPen size={17} /></button></Tooltip>}</div>
      ...
      {mobileActionsOpen && <div ref={actionsPopoverRef} id={mobileActionsId} role={condensedActions ? "dialog" : "region"} aria-label={`${locale === "fr" ? "Actions pour" : "Actions for"} ${track.title}`} className={cn("parigo-track-row__mobile-actions bg-[var(--surface)] px-2.5 pb-2.5 pt-2.5", condensedActions ? "parigo-track-actions-popover absolute right-2 top-[calc(100%+.25rem)] w-[min(18rem,calc(100vw-2rem))] border border-[var(--line-strong)]" : "border-t border-[var(--line)] lg:hidden")}>
        ...
        <div className={cn("grid grid-cols-2 gap-1.5", !condensedActions && "sm:grid-cols-3")}>
          ...
          <SimilarTracksButton trackId={track.id} trackTitle={track.title} mobileAction onNavigate={() => setMobileActionsOpen(false)} />
