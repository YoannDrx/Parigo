      ...
      >
        {isStowed ? <div className="parigo-player__stowed flex w-16 flex-col">
          <div className="parigo-player__stowed-cover relative h-16 w-16 overflow-hidden bg-[#101410]">
            ...
            <Tooltip label={isPlaying ? t("common.pause") : t("common.play")}><button type="button" onClick={(event) => { event.stopPropagation(); if (isPlaying) pause(); else resume(); }} className="parigo-player__stowed-play absolute inset-0 m-auto grid h-9 w-9 place-items-center bg-[var(--signal)] text-[#0c120d] shadow-[0_0_0_4px_rgba(8,10,8,.45)] transition hover:scale-105 hover:bg-white" aria-label={isPlaying ? t("common.pause") : t("common.play")}>
            ...
            <Tooltip label={locale === "fr" ? "Déployer le lecteur" : "Restore player"}><button type="button" onClick={(event) => { event.stopPropagation(); setPlayerView("docked"); }} className="parigo-player__stowed-expand absolute right-0 top-0 grid h-6 w-6 place-items-center bg-black/72 text-white/82 transition hover:bg-white hover:text-black" aria-label={locale === "fr" ? "Déployer le lecteur" : "Restore player"}><Maximize2 size={11} /></button></Tooltip>
          </div>
        ...
        <AnimatePresence initial={false}>
          {isExpanded && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: .32, ease: [.22, 1, .36, 1] }} className="parigo-player__expanded relative overflow-y-auto overscroll-contain border-t border-white/10">
            <div className="grid gap-5 px-4 py-4 sm:grid-cols-[auto_1fr] md:px-6">
              <div className="flex items-center gap-1 sm:flex-col sm:items-stretch sm:border-r sm:border-white/10 sm:pr-5 lg:border-r-0 lg:pr-0">
