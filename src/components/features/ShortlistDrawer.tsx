...
import { ArrowDown, ArrowUp, Check, ListPlus, Play, Save, Trash2, X } from "lucide-react";
        ...
        {isOpen && <>
          ...
          <motion.aside role="dialog" aria-modal="true" aria-label={t("search.shortlist")} className="parigo-drawer parigo-drawer--right fixed bottom-0 right-0 top-0 z-[80] flex w-full max-w-lg flex-col bg-[var(--background)] p-5 text-[var(--foreground)] md:p-7" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 28, stiffness: 260 }}>
            ...
            <div className="flex-1 overflow-y-auto py-4">{items.length ? items.map((item, index) => {
              ...
              return (
                <article key={item.track.id} className="group grid grid-cols-[3rem_minmax(0,1fr)_auto] items-center gap-3 border-b border-[var(--line)] py-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <Link href={trackTarget} onClick={() => setOpen(false)} className="block truncate text-sm font-semibold transition-colors hover:text-[var(--signal-strong)] focus-visible:text-[var(--signal-strong)] focus-visible:outline-none">{item.track.title}</Link>
                  ...
                  <div className="flex">
                    <button type="button" disabled={index === 0} onClick={() => move(item.track.id, -1)} className="flex h-9 w-8 items-center justify-center text-[var(--text-muted)] transition hover:bg-[var(--surface-soft)] hover:text-[var(--foreground)] disabled:opacity-20" aria-label={locale === "fr" ? "Monter" : "Move up"}><ArrowUp size={14} /></button>
                    <button type="button" disabled={index === items.length - 1} onClick={() => move(item.track.id, 1)} className="flex h-9 w-8 items-center justify-center text-[var(--text-muted)] transition hover:bg-[var(--surface-soft)] hover:text-[var(--foreground)] disabled:opacity-20" aria-label={locale === "fr" ? "Descendre" : "Move down"}><ArrowDown size={14} /></button>
                    <button type="button" onClick={() => remove(item.track.id)} className="flex h-9 w-8 items-center justify-center text-[var(--text-muted)] transition hover:bg-[var(--surface-soft)] hover:text-[var(--danger)]" aria-label={`${t("search.removeShortlist")} : ${item.track.title}`}><Trash2 size={15} /></button>
                  </div>
                </article>
              );
            }) : <div className="mx-auto max-w-sm py-12 text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-[var(--signal-strong)]/40 text-[var(--signal-strong)]"><ListPlus size={25} /></span><h3 className="mt-5 text-2xl">{locale === "fr" ? "Votre sélection est vide" : "Your selection is empty"}</h3><p className="mt-3 text-sm leading-6 text-[var(--text-muted)]">{locale === "fr" ? "Dans une liste de pistes, utilisez le bouton + entouré de vert. Les pistes ajoutées apparaîtront ici, dans l’ordre de votre choix." : "In any track list, use the green outlined + button. Added tracks will appear here in your chosen order."}</p><ol className="mt-7 grid gap-2 text-left text-xs text-[var(--text-muted)]"><li><b className="mr-2 text-[var(--foreground)]">01</b>{locale === "fr" ? "Ajoutez des pistes depuis la recherche, un album ou une playlist." : "Add tracks from search, an album or a playlist."}</li><li><b className="mr-2 text-[var(--foreground)]">02</b>{locale === "fr" ? "Réordonnez et écoutez votre sélection." : "Reorder and listen to your selection."}</li><li><b className="mr-2 text-[var(--foreground)]">03</b>{locale === "fr" ? "Connectez-vous pour la convertir en playlist Parigo." : "Sign in to convert it to a Parigo playlist."}</li></ol></div>}</div>
            {message && <div role={saved ? "status" : "alert"} className={`mb-3 grid grid-cols-[1.75rem_minmax(0,1fr)_1.75rem] items-center border p-3 text-center text-xs leading-5 ${saved ? "border-emerald-300 bg-emerald-50 text-emerald-900" : "border-amber-300 bg-amber-50 text-amber-900"}`}><span aria-hidden="true" /><div className="min-w-0"><p>{message}</p>{messageRequestId && <p className="mt-1 font-mono text-[.62rem] opacity-65">{locale === "fr" ? "Référence" : "Reference"} : {messageRequestId}</p>}</div><button type="button" onClick={() => { setMessage(""); setMessageRequestId(""); }} className="flex h-7 w-7 shrink-0 items-center justify-center border border-current/25 transition hover:bg-black/5" aria-label={locale === "fr" ? "Fermer le message" : "Dismiss message"}><X size={13} /></button></div>}
            {session?.user && items.length > 0 && <div className="mb-3 grid gap-3 border border-[var(--line)] bg-[var(--surface)] p-3"><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold">{locale === "fr" ? "Enregistrer la shortlist" : "Save shortlist"}</p><CueSheetButton compact title={locale === "fr" ? "Sélection Parigo" : "Parigo selection"} trackIds={tracks.map((track) => track.id)} /></div><div role="group" aria-label={locale === "fr" ? "Destination de la shortlist" : "Shortlist destination"} className="grid grid-cols-2 bg-[var(--surface-soft)] p-1 text-xs font-semibold"><button type="button" aria-pressed={saveMode === "new"} onClick={() => setSaveMode("new")} className={`min-h-9 px-2 ${saveMode === "new" ? "bg-[var(--surface)] text-[var(--foreground)]" : "text-[var(--text-muted)]"}`}>{locale === "fr" ? "Nouvelle playlist" : "New playlist"}</button><button type="button" aria-pressed={saveMode === "existing"} onClick={() => setSaveMode("existing")} className={`min-h-9 px-2 ${saveMode === "existing" ? "bg-[var(--surface)] text-[var(--foreground)]" : "text-[var(--text-muted)]"}`}>{locale === "fr" ? "Playlist existante" : "Existing playlist"}</button></div>{saveMode === "new" ? <><label htmlFor="shortlist-playlist-title" className="sr-only">{locale === "fr" ? "Nom de la nouvelle playlist" : "New playlist name"}</label><input id="shortlist-playlist-title" value={playlistTitle} onChange={(event) => setPlaylistTitle(event.target.value)} maxLength={160} className="h-11 border border-[var(--line)] bg-[var(--surface)] px-3 text-sm outline-none transition focus:border-[var(--signal-strong)]" /><button type="button" onClick={() => void saveToPlaylist()} disabled={saving || !playlistTitle.trim()} className="flex min-h-11 items-center justify-center gap-2 bg-[var(--foreground)] px-3 text-sm font-semibold text-[var(--background)] disabled:opacity-50">{saving ? <ParigoLoader size="icon" label={locale === "fr" ? "Création de la playlist" : "Creating playlist"} /> : <Save size={16} />}{locale === "fr" ? "Créer la playlist" : "Create playlist"}</button></> : <><Select value={targetPlaylist} onValueChange={setTargetPlaylist} ariaLabel={locale === "fr" ? "Playlist existante" : "Existing playlist"} className="min-w-0" placement="top" listboxClassName="max-h-[min(15rem,42vh)]" options={[{ value: "", label: locale === "fr" ? "Choisir une playlist…" : "Choose a playlist…" }, ...playlists.map((playlist) => ({ value: playlist.id, label: playlist.title }))]} /><button type="button" disabled={!targetPlaylist || saving} onClick={() => void saveToPlaylist(targetPlaylist)} className="flex min-h-11 items-center justify-center gap-2 border border-[var(--line)] px-3 text-sm font-semibold transition hover:border-[var(--signal-strong)] disabled:opacity-40">{saving ? <ParigoLoader size="icon" label={locale === "fr" ? "Ajout à la playlist" : "Adding to playlist"} /> : <Save size={16} />}{locale === "fr" ? "Ajouter à la playlist" : "Add to playlist"}</button></>}</div>}
