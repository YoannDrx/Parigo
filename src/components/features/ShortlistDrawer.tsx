      ...
      <AnimatePresence>
        {isOpen && <>
          ...
          <motion.aside role="dialog" aria-modal="true" aria-label={t("search.shortlist")} className="shortlist-drawer parigo-drawer parigo-drawer--right fixed bottom-0 right-0 top-0 z-[80] flex w-full max-w-lg flex-col bg-[var(--background)] p-5 text-[var(--foreground)] md:p-7" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 28, stiffness: 260 }}>
            ...
            {similarityCapabilities.data?.track.enabled && items.length > 0 && <Link href={`${localizedPath("/search")}?mode=ai&source=track&seeds=${encodeURIComponent(items.slice(0, 10).map((item) => item.track.id).join(","))}`} onClick={() => setOpen(false)} className="mb-3 flex min-h-12 items-center justify-center gap-2 border border-[var(--ai-search)] text-sm font-semibold text-[var(--ai-search)] transition hover:bg-[color-mix(in_srgb,var(--ai-search)_9%,var(--surface))]"><Sparkles size={16} />{locale === "fr" ? "Trouver des pistes similaires" : "Find similar tracks"}</Link>}
