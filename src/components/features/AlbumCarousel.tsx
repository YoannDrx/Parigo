...
export function AlbumCarousel({
  ...
  const gap = visibleItems > 2 ? 24 : 16;
  const itemWidth = containerWidth > 0
    ? (containerWidth - gap * (visibleItems - 1)) / visibleItems
    : 200;
  const totalWidth = albums.length * itemWidth + (albums.length - 1) * gap;
  const maxScroll = Math.max(0, totalWidth - containerWidth);
  const totalPages = Math.ceil(albums.length / visibleItems);
  const pageWidth = containerWidth > 0 ? containerWidth + gap : 0;
        ...
        >
          {albums.map((album, index) => (
            <motion.div
              ...
              style={{ width: itemWidth > 0 ? itemWidth : `calc(${100 / visibleItems}% - ${((visibleItems - 1) * gap) / visibleItems}px)` }}
            ...
            >
              ...
              >
                <AlbumCard album={album} priority={index < visibleItems} />
              </div>
            </motion.div>
          ))}
        </motion.div>
