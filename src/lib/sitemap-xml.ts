...
function safeLastModified(value: string | undefined, now = Date.now()) {
  if (!value) return undefined;
  const timestamp = Date.parse(value);
  return Number.isFinite(timestamp) && timestamp <= now ? value : undefined;
}

export function renderUrlSet(entries: SitemapEntry[]): string {
  const urls = entries.flatMap((entry) => {
    const lastModified = safeLastModified(entry.lastModified);
    return [
      { location: entry.fr },
      { location: entry.en },
    ].map(({ location }) => [
