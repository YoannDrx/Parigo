...
export default function TagsPage() {
  ...
  const rename = async (tag: MemberTag) => {
    const next = window.prompt(locale === "fr" ? "Nouveau nom du tag" : "New tag name", tag.name)?.trim();
    if (!next || next === tag.name) return;
    const response = await fetch(`/api/user/tags/${encodeURIComponent(tag.id)}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: next }) });
    if (response.ok) await loadTags();
  };

  const remove = async (tag: MemberTag) => {
    if (!window.confirm(locale === "fr" ? `Supprimer le tag « ${tag.name} » ?` : `Delete “${tag.name}”?`)) return;
    const response = await fetch(`/api/user/tags/${encodeURIComponent(tag.id)}`, { method: "DELETE" });
    if (response.ok) { if (selected === tag.id) { setSelected(null); setTracks([]); } await loadTags(); }
  };
  ...
  return <div className="space-y-8">
    ...
    {loading ? <Loader2 className="animate-spin" /> : <div className="grid gap-6 lg:grid-cols-[minmax(260px,360px)_1fr]">
      <section className="border border-[var(--line)]">
        {tags.length === 0 ? <p className="p-6 text-sm text-[var(--text-muted)]">{locale === "fr" ? "Aucun tag pour le moment." : "No tags yet."}</p> : tags.map((tag) => <div key={tag.id} className={`flex items-center gap-3 border-b border-[var(--line)] p-3 last:border-b-0 ${selected === tag.id ? "bg-[var(--foreground)] text-[var(--background)]" : ""}`}><button type="button" onClick={() => setSelected(tag.id)} className="flex min-h-10 flex-1 items-center gap-3 text-left"><Tag size={16} /><span className="flex-1 text-sm">{tag.name}</span><span className="font-mono text-[.62rem] opacity-55">{tag.trackCount}</span></button><button type="button" onClick={() => void rename(tag)} aria-label={locale === "fr" ? "Renommer" : "Rename"} className="flex h-10 w-10 items-center justify-center"><Pencil size={15} /></button><button type="button" onClick={() => void remove(tag)} aria-label={locale === "fr" ? "Supprimer" : "Delete"} className="flex h-10 w-10 items-center justify-center"><Trash2 size={15} /></button></div>)}
      </section>
      <section className="min-h-72 border border-[var(--line)] p-5">
