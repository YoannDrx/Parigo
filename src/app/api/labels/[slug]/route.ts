...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(
      { data: { label: { ...apiLabel(localizeLabel({ ...label, albumCount: albums.total }, language)), albums: albums.items.map(apiAlbum), trackCount: albums.items.reduce((sum, album) => sum + album.trackCount, 0) } }, meta: { requestId: id } },
