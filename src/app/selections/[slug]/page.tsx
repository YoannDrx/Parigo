  ...
  return (
    <div className="page-shell min-h-screen">
      ...
      <main className="pb-28 pt-[74px]">
        <header className="border-b border-[var(--line)] px-[var(--space-page-gutter)] py-[var(--space-section-y-large)]">
        ...
        <section className="mx-auto max-w-[1500px] px-[var(--space-page-gutter)] py-[var(--space-section-y)]" aria-labelledby="selection-results">
          <div className="mb-[var(--space-heading-content)] flex items-end justify-between gap-6"><SignedTitle as="h2" id="selection-results" className="font-[var(--font-editorial)] text-5xl font-normal">{locale === "fr" ? "Albums sélectionnés" : "Selected albums"}</SignedTitle><span className="font-mono text-xs text-[var(--text-muted)]">{albums.length} albums</span></div>
          <div className="grid grid-cols-1 gap-x-[var(--space-grid-x)] gap-y-[var(--space-grid-y)] sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">{albums.map((album) => <AlbumCard key={album.id} album={album} headingLevel={3} />)}</div>
        </section>
        <section className="border-y border-[var(--line)] bg-[var(--surface-soft)] px-[var(--space-page-gutter)] py-[var(--space-section-y)]"><div className="mx-auto grid max-w-[1500px] gap-[var(--space-block-gap)] md:grid-cols-2"><article><h2 className="text-2xl font-semibold">{locale === "fr" ? "Usages à l’image" : "Uses on screen"}</h2><p className="mt-4 leading-7 text-[var(--text-muted)]">{content.uses}</p></article><article><h2 className="text-2xl font-semibold">{locale === "fr" ? "Conseil de sélection" : "Selection advice"}</h2><p className="mt-4 leading-7 text-[var(--text-muted)]">{content.advice}</p></article></div></section>
        <section className="mx-auto max-w-[1500px] px-[var(--space-page-gutter)] py-[var(--space-section-y)]"><SignedTitle as="h2" className="text-3xl font-semibold">{locale === "fr" ? "Sélections connexes" : "Related selections"}</SignedTitle><div className="mt-[var(--space-heading-content)] flex flex-wrap gap-3">{related.map((item) => item && <Link key={item.key} href={localizedPath(locale, `/selections/${item.content[locale].slug}`)} className="min-h-11 rounded-full border border-[var(--line)] px-5 py-3 text-sm font-semibold hover:border-[var(--signal-strong)]">{item.content[locale].title}</Link>)}</div><Link href={localizedPath(locale, "/contact")} className="mt-[var(--space-block-gap)] inline-flex min-h-12 items-center rounded-full bg-[var(--foreground)] px-6 font-semibold text-[var(--background)]">{locale === "fr" ? "Parler de votre licence" : "Discuss your licence"}</Link></section>
      </main>
      <Footer />
    </div>
  );
